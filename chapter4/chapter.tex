\chapter{Improving Southern Ocean boundary layer cloud parameterisation in the HadGEM3-GA7.1/UM11.4 general circulation model}
...
and the two Sc cloud heights coincides with the neutral buoyancy level of a dry and moist (respectively) air
...
in contemporary GCMs \citep{li2013}. Clouds in these models are parameterised by subgrid--scale parameterisation schemes. Cloud typically varies at scales much smaller
...
model resolution and have to be parameterised. The nature of these parameterisations means that they inevitably lead to large uncertainties. An alternative to such schemes are cloud resolving models
(CRMs), which operate at a much higher horizontal spatial resolution (on the order of 1 km)
\citep{guichard2017,satoh2019}.
However, at present it is unfeasible to use these models for long-term climate
projections due to their computational demands. Therefore, a continued improvement of
existing cloud schemes is needed.

Cloud parameterisation in GCMs is traditionally improved by intercomparing with
...
parameterisation scheme \citep{morcrette2010}. The presence of compensating
errors can also make the model perform worse when only one of the errors is
corrected \citep{hourdin2017,schuddeboom2019}.

Here, we focus on the Southern Ocean (SO) boundary layer parameterisations in the
...
This follows the findings of \cite{kuma2019} of underestimated cloud occurrence
below 2 km above sea level (ASL) in GA7.1 relative to a comprenesive set of
ship-based observations.
The GA7.1 model is based the  UK Met Office
Unified Model (UM), which is also used for operational numerical weather
prediction (NWP). HadGEM3 (GC3.1) has participated in the 6$^{\rm th}$ Coupled Model Intercomparison Project (CMIP6). As shown by \cite{kuma2019}, cloud
...
The UM parameterises clouds using the prognostic PC2 scheme
...
% This could be shortened and tabulated, for easier readability. OM.
...
dynamics is effectively prescribed while parameterised fields
...
The parameterisations in the HadGEM3-GA7.1/UM11.4 are described in Sect. \ref{sec:parametrisations}.
...
The UM11.4 relies on the parameterisation of subgrid-scale
processes,
which cannot be represented by the atmospheric dynamics on the relatively
coarse grid of the model. The parameterisations directly affecting cloud
are the convection, boundary layer, and large-scale cloud (PC2) schemes
and the related parameterisation of surface fluxes in the surface component of
...
of parameterised processes. This interplay means that it is typically not obvious from model
evaluation studies which process is responsible for a bias in cloud occurrence
or TOA radiation balance. The parameterisations outlined above operate independently
on each individual horizontal grid cell of the model.
Table \ref{tab:parametrisations} summarises the cloud-related
parameterisation schemes, described in a greater detail below with a specific
...
the grid mean, parameterised by a critical Relative Humidity (RH) \citep{gregory2002}.
...
with non--local mixing performed by the mass flux parameterisation in the
convection scheme.
...
version 6, which is based on a mass flux parameterisation described by \cite{gregory1990}.
The convection scheme parameterises convective plumes covering a fraction of the grid cell,
...
\subsection{JULES surface flux parameterisation}
\label{sec:jules}

JULES is the surface parameterisation model used in conjunction with the UM.
Among other processes, JULES parameterises fluxes of momentum, heat and moisture from
...
Charnock's coefficient $a$ is parameterised by a linear
...
parameterisation operation most
...
In the UM11.4 this is parameterised by the BL scheme first-order turbulence scheme
in the surface and Sc layers and the convection scheme mass flux parameterisation
...
As noted by \cite{lock2000}, this vertical separation of the parameterisation
...
this vertical separation of the two parameterisation schemes may be responsible for the
...
in Fig. \ref{fig:examples} and \ref{fig:examples-cont}; see also \cite{kuma2019}).
...
As explained in Sect. \ref{sec:schemes}, the mass flux parameterisation
...
may be an indication that the mass flux parameterisation operates more quickly 
than the surface turbulence with respect to moisture transport and therefore these two processes are not currently well-tuned to operate together. This is happening in the unmodified
...
%Can we quantify this relative to observations or plot differences to highlight this point? AJM
...
parameterisation cannot by itself solve the problem further. By a more extreme
...
turbulence with the mass flux parameterisation, whereby not enough air mass
...
applied to the backscatter profiles \citep{kuma2020}.
...
the frequent fog occurrence previously discussed in \cite{kuma2019},
...
% so why don't we see a bigger difference consistently?
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{chapter4/fig/examples_hur_mcu.pdf}
...
\caption[Boundary layer type histograms]{Boundary layer type histograms of types I.--VI. \citep{lock2000} in the UM11.4cnt
